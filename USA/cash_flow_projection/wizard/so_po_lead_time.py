...
class LeadTimeSettings(models.TransientModel):
    ...
    so_lead_time = fields.Integer(string='Due date for SO',
                                  default=lambda self: self.env.user.company_id.customer_payment_lead_time)
    ...
    def set_so_lead_time(self):
        ...
        self.env.user.company_id.customer_payment_lead_time = self.so_lead_time
        self.env.user.company_id.vendor_payment_lead_time = self.po_lead_time
        return {
