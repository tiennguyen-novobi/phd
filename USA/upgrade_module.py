...
def upgrade_module_list(check_version, result):
	...
	if len(db_list_final) == 0:
		...
		if len(module_list_upgrade_final) != 0 and len(module_list_install_final) != 0:
			...
			print ("Database: %s" % db_list_final)		
			file = open("./odoo_upgrade.sh", "w")
			file.write(odoo_upgrade + "/opt/odoo/odoo/odoo-bin --config /etc/odoo/odoo.conf --stop-after-init -i %s -u %s -d %s" % (module_list_install_final, module_list_upgrade_final, db_list_final))
			file.close()
			...
			print ("Database: %s" % db_list_final)		
			file = open("./odoo_upgrade.sh", "w")
			file.write(odoo_upgrade + "/opt/odoo/odoo/odoo-bin --config /etc/odoo/odoo.conf --stop-after-init -u %s -d %s" % (module_list_upgrade_final, db_list_final))
			file.close()
			...
			print ("Database: %s" % db_list_final)		
			file = open("./odoo_upgrade.sh", "w")
			file.write(odoo_upgrade + "/opt/odoo/odoo/odoo-bin --config /etc/odoo/odoo.conf --stop-after-init -i %s -d %s" % (module_list_install_final, db_list_final))
			file.close()
		else:
			print ("Do not have anythings upgrade")
			file = open("./odoo_upgrade.sh", "w")
