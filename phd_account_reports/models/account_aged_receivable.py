...
class PHDAccountAgedReceivable(models.AbstractModel):
    _inherit = "account.aged.receivable"

    def _get_columns_name(self, options):
        columns = super(PHDAccountAgedReceivable, self)._get_columns_name(options)
        if columns and len(columns) >= 1:
            columns.insert(1, {'name': _("Document Date"), 'class': 'date', 'style': 'white-space:nowrap;'},)
        return columns
                ...
                for line in amls[values['partner_id']]:
                    ...
                    vals = {
                        ...
                        'columns': [{'name': v} for v in
                                    [format_date(self.env, aml.date), format_date(self.env, aml.date_maturity), aml.journal_id.code,
...
class PHDAccountAgedPayable(models.AbstractModel):
    _inherit = "account.aged.payable"

    def _get_columns_name(self, options):
        columns = super(PHDAccountAgedPayable, self)._get_columns_name(options)
        if columns and len(columns) >= 1:
            columns.insert(1, {'name': _("Document Date"), 'class': 'date', 'style': 'white-space:nowrap;'},)
        return columns
            ...
            if 'partner_%s' % (values['partner_id'],) in options.get('unfolded_lines'):
                for line in amls[values['partner_id']]:
                    ...
                    vals = {
                        ...
                        'parent_id': 'partner_%s' % (values['partner_id'],),
                        'columns': [{'name': v} for v in
                                    [format_date(self.env, aml.date), format_date(self.env, aml.date_maturity),
                                     aml.journal_id.code,
                                     aml.account_id.display_name, format_date(self.env, aml.expected_pay_date)]] +
                                   [{'name': self.format_value(sign * v, blank_if_zero=True), 'no_format': sign * v} for
                                    v in [line['period'] == 6 - i and line['amount'] or 0 for i in range(7)]],
